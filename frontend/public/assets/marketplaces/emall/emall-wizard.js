...
html += '</div>';
    ...
    function render() {
        ...
        if (wizard.currentStepIndex === 0) {
            ...
            var propsFillDisabled = wizard.categoryPropertiesFilling ? ' disabled' : '';
            var propsContinueLabel = wizard.categoryPropertiesFilling ? t('propertiesFillLoading') : t('continue');
            html += '<button class="btn btn-secondary" id="prevBtn"' + (wizard.categoryPropertiesFilling ? ' disabled' : '') + '>' + t('prev') + '</button>';
            html += '<div><button class="btn btn-primary" id="continuePropertiesBtn"' + propsFillDisabled + '>' + propsContinueLabel + '</button></div>';
        } else if (wizard.currentStepIndex === 5) {
            html += '<button class="btn btn-secondary" id="prevBtn">' + t('prev') + '</button>';
    ...
    function fetchBrandCountryAndDirectories() {
        ...
        Promise.all([
            ...
            wizard.brandCountryPredicted = { brand: (pred && pred.brand) ? pred.brand : '', country: (pred && pred.country) ? pred.country : '', brand_resolved: (pred && pred.brand_resolved) ? pred.brand_resolved : null, country_resolved: (pred && pred.country_resolved) ? pred.country_resolved : null };
            // Do not preload all brands (huge directory). We'll search on demand.
            wizard.brandsList = [];
            wizard.countriesList = countries;

            // If backend resolved IDs, auto-select immediately. Otherwise, show manual selection UI.
            if (pred && pred.brand_resolved && pred.brand_resolved.id != null) {
                wizard.brandSelected = { id: pred.brand_resolved.id, name: pred.brand_resolved.name || '' };
                wizard.brandManual = false;
            } else {
                wizard.brandSelected = null;
                wizard.brandManual = true;
            }
            if (pred && pred.country_resolved && pred.country_resolved.id != null) {
                wizard.countrySelected = { id: pred.country_resolved.id, name: pred.country_resolved.name || '' };
                wizard.countryManual = false;
            } else {
                wizard.countrySelected = null;
                wizard.countryManual = true;
            }

            // Backward-compatible fallback: if not resolved, try to resolve predicted brand via server-side search.
            if (!wizard.brandSelected) {
                ...
                if (qb) {
                    ...
                    apiFetch('/emall/brands/search' + qs).then(function(bres) {
                        ...
                        if (brandMatch) wizard.brandSelected = { id: brandMatch.id, name: brandMatch.name };
                        render();
                    }).catch(function() {
                        // ignore
                    });
    ...
    function attachEventHandlers() {
        ...
        var continuePropertiesBtn = document.getElementById('continuePropertiesBtn');
        if (continuePropertiesBtn) {
            continuePropertiesBtn.onclick = function() {
                if (wizard.categoryPropertiesFilled && wizard.categoryPropertiesFilled.length > 0) {
                    showStatus('saving', t('saving'));
                    saveDraft(true).then(function() {
                        showStatus('saved', t('saved'));
